...
function normalizeCustomTraceTags(value: unknown): string[] {
...
const langfuseTracePlugin: OpenClawPlugin = {
  id: "openclaw-exporter-to-langfuse",
  name: "OpenClaw Exporter to Langfuse",
  version: PLUGIN_VERSION,
  description:
    "Export OpenClaw AI agent execution traces to Langfuse via OpenTelemetry",
  ...
  activate(api: OpenClawPluginApi) {
    ...
    if (!endpoint) {
      api.logger.error(
        "[LangfuseTrace] Missing required configuration: 'endpoint' must be provided",
      );
      return;
    }
    if (!headers || !headers["Authorization"]) {
      api.logger.error(
        "[LangfuseTrace] Missing required configuration: 'headers.Authorization' must be provided",
      );
      return;
    }
    ...
    const skillTagDetectionCfg = resolveSkillTagDetectionConfig(pluginConfig);
    if (config.debug) {
      api.logger.info(
        `[LangfuseTrace] Skill tag detection config: ${JSON.stringify(skillTagDetectionCfg)}`,
      );
      ...
      if (pluginConfig.skillTaggingEnabled === true && skillTagDetectionCfg && skillTagDetectionCfg.skillsRoots.length === 0) {
        api.logger.warn(
          "[LangfuseTrace] Skill tag detection is enabled but no skills roots were resolved. Set 'skillsRoots' in plugin config or OPENCLAW_SKILLS_ROOT env.",
        );
      }
    }
    ...
    const sweepStaleContexts = () => {
      ...
      for (const [traceId, pending] of pendingAssistantByTraceId) {
        if (now - pending.createdAt > PENDING_ASSISTANT_TTL_MS) {
          ...
          if (config.debug) {
            api.logger.warn(
              `[LangfuseTrace] Dropped stale pending assistant message: traceId=${traceId}`,
            );
          }
        }
      }
    ...
    const bindRealRunId = (ctx: TraceContext, runId: string, hookName: string) => {
      ...
      if (existing && existing !== ctx) {
        if (config.debug) {
          api.logger.warn(
            `[LangfuseTrace] Skip runId rebind due to existing mapping: hook=${hookName}, oldRunId=${ctx.runId}, realRunId=${realRunId}`,
          );
        }
        return;
      }
      ...
      if (!isTemporaryRunId(oldRunId)) {
        if (config.debug) {
          api.logger.warn(
            `[LangfuseTrace] Skip runId rebind because current runId is already stable: hook=${hookName}, runId=${oldRunId}, incoming=${realRunId}`,
          );
        }
        return;
      }
      ...
      if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] Rebound temporary runId: hook=${hookName}, oldRunId=${oldRunId}, realRunId=${realRunId}`,
        );
      }
    };
    ...
    const consumePendingToolCall = (event: AfterToolCallEvent): PendingToolCall | undefined => {
      ...
      if (fallback && fallbackKey) {
        ...
        if (config.debug) {
          api.logger.warn(
            `[LangfuseTrace] Tool call fallback match used: tool=${event.toolName}, runId=${eventRunId || "-"}, eventToolCallId=${eventToolCallId || "-"}`,
          );
        }
      }
      return fallback;
    ...
    const getOrCreateContext = (
      ...
      const linkRecentUserContext = () => {
        ...
        if (config.debug) {
          api.logger.info(
            `[LangfuseTrace] LINKING agent to user context: hook=${hookName}, agentChannel=${rawChannelId}, userChannel=${channelId}, traceId=${linked.traceId}`,
          );
        }
        return linked;
      };
      ...
      if (
        ...
        if (config.debug) {
          api.logger.info(
            `[LangfuseTrace] Rotating user context for new message: channelId=${channelId}, previousRunId=${activeCtx.runId}`,
          );
        }
        activeCtx = undefined;
      ...
      if (
        ...
        if (config.debug) {
          api.logger.info(
            `[LangfuseTrace] New stable run detected on active channel; creating fresh context: hook=${hookName}, channelId=${channelId}, oldRunId=${activeCtx.runId}, newRunId=${resolvedIncomingRunId}`,
          );
        }
        activeCtx = undefined;
      ...
      if (
        ...
        if (existingReal && existingReal !== activeCtx) {
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Switching temporary context to existing real run context: hook=${hookName}, channelId=${channelId}, tempRunId=${activeCtx.runId}, realRunId=${resolvedIncomingRunId}`,
            );
          }
      ...
      if (!activeCtx) {
        ...
        if (config.debug) {
          api.logger.info(
            `[LangfuseTrace] NEW TraceContext: hook=${hookName}, channelId=${channelId}, runId=${effectiveRunId}, traceId=${activeCtx.traceId}`,
          );
        }
      } else if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] REUSING TraceContext: hook=${hookName}, channelId=${channelId}, traceId=${activeCtx.traceId}`,
        );
      }
    ...
    const exportPendingLlmSpan = async (
      ...
      if (safeEndTime !== now && config.debug) {
        api.logger.warn(
          `[LangfuseTrace] Corrected negative LLM span duration: runId=${ctx.runId}, traceId=${ctx.traceId}, start=${startTime}, end=${now}`,
        );
      }
      ...
      if (effectiveInputMessages) {
        ...
        llmAttrs["langfuse.observation.input"] = truncateAttr(effectiveInputMessages);
        // llmAttrs["gen_ai.prompt"] = truncateAttr(effectiveInputMessages);
      }
      if (params.outputContent !== undefined || params.outputTexts.length > 0) {
        const outputMsgs = formatAssistantOutputMessages(
          params.outputContent,
          params.outputTexts,
          stopReason,
        );
        // llmAttrs["gen_ai.output.messages"] = outputMsgs;
        // Langfuse observation output
        llmAttrs["langfuse.observation.output"] = truncateAttr(outputMsgs);
        // llmAttrs["gen_ai.completion"] = truncateAttr(outputMsgs);
      }
      ...
      if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] Exported segmented LLM span: ${provider}/${model}, duration=${safeEndTime - startTime}ms, reason=${stopReason}`,
        );
      }
    };
    ...
    const processAssistantMessageForContext = async (
      ...
      if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] Processed assistant message for segmented LLM: runId=${ctx.runId}, toolCalls=${toolCalls.length}, stopReason=${stopReason}`,
        );
      }
    };
    ...
    const ensureEntrySpan = async (
      ...
      if (ctx.userInput) {
        const inputStr = typeof ctx.userInput === "string"
          ? ctx.userInput
          : JSON.stringify(ctx.userInput);
        rootAttrs["langfuse.trace.input"] = truncateAttr(inputStr);
      }
      if (customTraceTags.length > 0) {
        rootAttrs["langfuse.tags"] = JSON.stringify(customTraceTags);
      }
      ...
      if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] Started root span: traceId=${ctx.traceId}, spanId=${ctx.rootSpanId}`,
        );
      }
    };
    ...
    const ensureAgentSpan = async (
      ...
      if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] Started agent span: ${agentId}, spanId=${ctx.agentSpanId}`,
        );
      }
    };
    ...
    const ensureStepSpan = async (
      ...
      if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] Started step span: round=${round}, spanId=${spanId}, runId=${ctx.runId}`,
        );
      }
    };
    ...
    const endStepSpan = (
      ...
      if (config.debug) {
        api.logger.info(
          `[LangfuseTrace] Ended step span: round=${stepRound}, reason=${finishReason}, spanId=${stepSpanId}`,
        );
      }
    };
    ...
    if (shouldHookEnabled("gateway_start")) {
      api.on("gateway_start", async (event: GatewayStartEvent) => {
        if (config.debug) {
          api.logger.info(`[LangfuseTrace] Hook gateway_start: port=${event.port}`);
        }
        const now = Date.now();
        ...
        if (customTraceTags.length > 0) {
          span.attributes["langfuse.tags"] = JSON.stringify(customTraceTags);
        }
        ...
        delete (span.attributes as Record<string, unknown>)["langfuse.session.id"];
        await exporter.export(span);
      });
    }
    ...
    if (shouldHookEnabled("session_start")) {
      api.on(
        ...
        async (event: SessionStartEvent, hookCtx: PluginHookContext) => {
          if (config.debug) {
            api.logger.info(`[LangfuseTrace] Hook session_start: trigger=${hookCtx.trigger}, sessionId=${event.sessionId}`);
          }
          ...
          // Store sessionId in context for all subsequent spans
          if (event.sessionId) {
          ...
          if (event.sessionId) {
            ...
            span.attributes["gen_ai.session.id"] = event.sessionId;
            // Add langfuse-specific session attribute
            span.attributes["langfuse.session.id"] = event.sessionId;
          }
          await exporter.export(span);
        },
      );
    ...
    if (shouldHookEnabled("session_end")) {
      api.on(
        ...
        async (event: SessionEndEvent, hookCtx: PluginHookContext) => {
          if (config.debug) {
            api.logger.info(`[LangfuseTrace] Hook session_end: trigger=${hookCtx.trigger}, sessionId=${event.sessionId}`);
          }
          const rawChannelId = resolveChannelId(hookCtx, event.sessionId);
          const { ctx, channelId } = getOrCreateContext(
            rawChannelId,
    ...
    if (shouldHookEnabled("message_sending")) {
      api.on(
        ...
        async (event: MessageSendingEvent, hookCtx: PluginHookContext) => {
          if (config.debug) {
            api.logger.info(`[LangfuseTrace] Hook message_sending: trigger=${hookCtx.trigger}, to=${event.to}`);
          }
    ...
    if (shouldHookEnabled("message_sent")) {
      api.on(
        ...
        async (event: MessageSentEvent, hookCtx: PluginHookContext) => {
          if (config.debug) {
            api.logger.info(`[LangfuseTrace] Hook message_sent: trigger=${hookCtx.trigger}, to=${event.to}, success=${event.success}`);
          }
        ...
        async (event: LlmInputEvent, hookCtx: PluginHookContext) => {
          ...
          if (pendingAssistant) {
            ...
            void enqueueTraceTask(ctx.traceId, async () => {
              if (config.debug) {
                api.logger.info(
                  `[LangfuseTrace] pending Assistant before_message_write usage: ${JSON.stringify(pendingAssistant.message.usage)}, message: ${JSON.stringify(pendingAssistant.message.content)}`,
                );
              }
              await processAssistantMessageForContext(ctx, channelId, pendingAssistant.message);
            }).catch((err) => {
              api.logger.warn(`[LangfuseTrace] replay pending assistant failed: ${String(err)}`);
            });
          }

          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Hook llm_input: trigger=${hookCtx.trigger}, ${event.provider}/${event.model}, runId=${event.runId}`,
            );
          }
        },
      );
    ...
    if (shouldHookEnabled("llm_output")) {
      api.on(
        ...
        async (event: LlmOutputEvent, hookCtx: PluginHookContext) => {
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Hook llm_output: trigger=${hookCtx.trigger}, runId=${event.runId}`,
            );
          }
          const rawChannelId = resolveChannelId(hookCtx);
          const { ctx, channelId } = getOrCreateContext(
          ...
          if (event.usage) {
            if (config.debug) {
              api.logger.info(
                `[LangfuseTrace] LLM output usage: ${JSON.stringify(event.usage)}`,
              );
            }
            ctx.lastLlmUsage = {
    ...
    if (shouldHookEnabled("before_message_write")) {
      api.on(
        ...
        (event: BeforeMessageWriteEvent, hookCtx: PluginHookContext) => {
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Hook before_message_write: trigger=${hookCtx.trigger}, role=${event.message?.role}`,
            );
          }
          ...
          if (rawChannelId.startsWith("agent/") && !anchoredCtx) {
            if (config.debug) {
              api.logger.warn(
                `[LangfuseTrace] Skip assistant message without active agent context: channelId=${rawChannelId}`,
              );
            }
            return;
          }
          ...
          if (!ctx.hasSeenLlmInput) {
            ...
            if (config.debug) {
              api.logger.warn(
                `[LangfuseTrace] Buffered assistant message waiting for llm_input: traceId=${ctx.traceId}, runId=${ctx.runId}`,
              );
            }
            return;
          }
          ...
          void enqueueTraceTask(ctx.traceId, async () => {
            if (config.debug) {
              api.logger.info(
                `[LangfuseTrace] Assistant before_message_write usage: ${JSON.stringify(message.usage)}, message: ${JSON.stringify(message.content)}`,
              );
            }
            await processAssistantMessageForContext(ctx, channelId, message);
          }).catch((err) => {
            api.logger.warn(`[LangfuseTrace] before_message_write segmented export failed: ${String(err)}`);
          });
        },
      );
    }
      ...
      api.on(
        ...
        async (
          ...
          if (rawChannelId.startsWith("agent/") && !anchoredCtx) {
            if (config.debug) {
              api.logger.warn(
                `[LangfuseTrace] Skip tool span without active agent context: tool=${event.toolName}, channelId=${rawChannelId}`,
              );
            }
            return;
          }
          ...
          if (!ctx.hasSeenLlmInput) {
            if (config.debug) {
              api.logger.warn(
                `[LangfuseTrace] Skip tool span before llm_input binding: tool=${event.toolName}, channelId=${channelId}, runId=${ctx.runId}`,
              );
            }
            return;
          }
          ...
          const pendingToolCallEntry: PendingToolCall = {
            runId,
          ...
          pendingToolCalls.set(buildPendingToolCallKey(toolCallId, runId), pendingToolCallEntry);
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Hook before_tool_call: trigger=${hookCtx.trigger}, tool=${event.toolName}, toolCallId=${toolCallId}, runId=${runId}, spanId=${pendingToolCallEntry.toolSpanId}`,
            );
          }
        },
      );
    ...
    if (shouldHookEnabled("after_tool_call")) {
      api.on(
        ...
        async (
          ...
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Hook after_tool_call: trigger=${hookCtx.trigger}, tool=${event.toolName}, toolCallId=${event.toolCallId}, runId=${event.runId}`,
            );
          }
          const pendingToolCallEntry = consumePendingToolCall(event);
          if (!pendingToolCallEntry) {
            return;
          }
          ...
          if (skillTagDetectionCfg) {
            const paramsForDetection =
              toolInput && typeof toolInput === "object" && !Array.isArray(toolInput)
                ? (toolInput as Record<string, unknown>)
                : event.params;
            const skillTags = detectSkillTagsFromToolCall(
              toolName,
              paramsForDetection,
              skillTagDetectionCfg,
            );
            if (skillTags.length > 0) {
              toolAttrs["langfuse.tags"] = JSON.stringify(skillTags);
              if (skillTagDetectionCfg.skillEmitMetadata) {
                toolAttrs["langfuse.observation.metadata.skills"] = JSON.stringify(skillTags);
              }
            }
          }
          ...
          if (hadWaitingTools) {
            ...
            if (toolBatchFinished) {
              ...
              if (config.debug) {
                api.logger.info(
                  `[LangfuseTrace] Prepared next segmented LLM start after tools: runId=${traceContext.runId}, tool=${toolName}, toolCallId=${toolCallId}`,
                );
              }
            }
          }
          ...
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Exported tool span: ${toolName}, toolCallId=${toolCallId}, duration=${now - toolStartTime}ms`,
            );
          }
        },
      );
    ...
    if (shouldHookEnabled("before_agent_start")) {
      api.on(
        ...
        async (
          ...
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Hook before_agent_start: trigger=${hookCtx.trigger}, agentId=${agentId}, channelId=${channelId}`,
            );
          }
    ...
    if (shouldHookEnabled("agent_end")) {
      api.on(
        ...
        async (event: AgentEndEvent, hookCtx: PluginHookContext) => {
          if (config.debug) {
            api.logger.info(
              `[LangfuseTrace] Hook agent_end: trigger=${hookCtx.trigger}, channelId=${resolveChannelId(hookCtx)}`,
            );
          }
          const rawChannelId = resolveChannelId(hookCtx);
          const { ctx, channelId } = getOrCreateContext(
          ...
          if (pendingAgentSpanId) {
            ...
            if (ctx.sessionId) {
              ...
              agentEndAttrs["langfuse.session.id"] = ctx.sessionId || channelId;
            }
            const agentInput = ctx.userInput;
            if (agentInput) {
              const inputStr = typeof agentInput === "string" ? agentInput : JSON.stringify(agentInput);
              // agentEndAttrs["gen_ai.input.messages"] = truncateAttr(
              //   JSON.stringify([{ role: "user", parts: [{ type: "text", content: String(agentInput) }] }]),
              // );
              // Langfuse observation input
              agentEndAttrs["langfuse.observation.input"] = truncateAttr(inputStr);
              // agentEndAttrs["gen_ai.prompt"] = truncateAttr(inputStr);
            }
            ...
            setTimeout(async () => {
              ...
              if (pendingAgentSpanId && agentEndAttrs) {
                if (finalOutput) {
                  ...
                  // );
                  // Langfuse observation output
                  agentEndAttrs["langfuse.observation.output"] = truncateAttr(outputStr);
                  // agentEndAttrs["gen_ai.completion"] = truncateAttr(outputStr);
                }
                ...
                if (config.debug) {
                  api.logger.info(
                    `[LangfuseTrace] Ended agent span: spanId=${pendingAgentSpanId}, duration=${event.durationMs}ms`,
                  );
                }
              }
              ...
              if (rootSpanStartTime) {
                ...
                if (resolvedSessionId) {
                  ...
                  rootEndAttrs["langfuse.session.id"] = resolvedSessionId;
                }
                if (userInput) {
                  ...
                  // );
                  // Langfuse observation input
                  rootEndAttrs["langfuse.observation.input"] = truncateAttr(inputStr);
                  // rootEndAttrs["gen_ai.prompt"] = truncateAttr(inputStr);
                }
                if (finalOutput) {
                  const outputStr = typeof finalOutput === "string" ? finalOutput : JSON.stringify(finalOutput);
                  ...
                  // );
                  // Langfuse observation output
                  rootEndAttrs["langfuse.observation.output"] = truncateAttr(outputStr);
                  // rootEndAttrs["gen_ai.completion"] = truncateAttr(outputStr);
                }
                exporter.endSpanById(
                  rootSpanId,
                  endTime,
                ...
                if (config.debug) {
                  api.logger.info(
                    `[LangfuseTrace] Ended root span: spanId=${rootSpanId}, duration=${endTime - rootSpanStartTime}ms, traceId=${traceId}`,
                  );
                }
              }
              ...
              exporter.endTrace();
                ...
                api.logger.warn(
                  `[LangfuseTrace] Deferred agent_end span close failed: ${String(err)}`,
                );
              }
            }, 100);
          } else {
    ...
    api.logger.info(
      `[LangfuseTrace] Plugin activated (endpoint: ${config.endpoint}, service: ${config.serviceName})`,
    );
  },
};
