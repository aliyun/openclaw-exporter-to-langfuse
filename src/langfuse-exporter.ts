...
/**
 * Exports OpenClaw trace spans to Langfuse via OTLP/Protobuf.
 *
...
export class LangfuseExporter {
  private readonly config: LangfuseTraceConfig;
  ...
  private async initialize(): Promise<void> {
    ...
    this.api.logger.info(`[LangfuseTrace] Initializing exporter → ${traceUrl}`);

    // Build headers, add Langfuse-specific header if using Authorization (Langfuse mode)
    const exporterHeaders: Record<string, string> = { ...this.config.headers };
    if (exporterHeaders["Authorization"]) {
      exporterHeaders["x-langfuse-ingestion-version"] = "4";
    }
    ...
    this.api.logger.info(
      `[LangfuseTrace] Exporter initialized (service=${this.config.serviceName})`,
    );
  }
  ...
  async startSpan(spanData: SpanData, spanId: string): Promise<void> {
    try {
      ...
      this.api.logger.error(`[LangfuseTrace] Failed to start span: ${err}`);
    }
  }
  ...
  private doStartSpan(spanData: SpanData, spanId: string): void {
    ...
    if (this.config.debug) {
      ...
      this.api.logger.info(
        `[LangfuseTrace] Started span: name=${spanData.name}, type=${spanData.type}, traceId=${sc.traceId}, spanId=${sc.spanId}`,
      );
    }
  }
  ...
  endSpanById(
    ...
    if (this.config.debug) {
      const sc = span.spanContext();
      this.api.logger.info(
        `[LangfuseTrace] Ended span: spanId=${spanId}, traceId=${sc.traceId}`,
      );
    }
  }
  ...
  async export(spanData: SpanData): Promise<void> {
    ...
    if (this.config.debug) {
      const sc = span.spanContext();
      this.api.logger.info(
        `[LangfuseTrace] Exported span: name=${spanData.name}, type=${spanData.type}, traceId=${sc.traceId}, spanId=${sc.spanId}`, 
      );
    }
  }
